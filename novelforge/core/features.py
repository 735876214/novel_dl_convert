...
FEATURE_LABELS = {
    ...
    "audio": "有声书播放",
    "annotations": "批注",
    # ⚠️ `bookmarks` 是给**书库管理页的能力矩阵**看的（说明哪类库支持书签），
    # **不要**拿它去藏阅读器里那个书签按钮：判隐显的轴是「这本书属于哪个库」，
    # 而前端 `hasFeature()` 判的是「侧栏当前选着哪个库」—— 选着漫画库时读电子书
    # 会因此把按钮藏掉（第 34 期浏览器冒烟实测）。本项目没有 bookmarks 的导航项 / 设置页。
    "bookmarks": "书签",
    "metadata": "元数据抓取",
    "authors": "作者元数据",
