...
> 真值源 = `.codebuddy/memory/`；逐期细节看同日日记，本文件只留**结构性决策与不变式**。**「bookorbit」默认指本项目**（只有对比上游时才区分）。
...
- `settingsNav.ts`/`router` 里「标 ready 但未注册组件」会 `console.error` → **注册表与组件必须同批改**（`libraries` 页已改回 `placeholder`，它只跳 `/tools/libraries`）。
- ⚠️ **Vue 模板不渲染 markdown**：模板里写 `**强调**` 会**原样显示星号** → 静态文本用 `<strong>`，JS 字符串/属性值直接别加星号。
...
- 与 `features.allows_setting` 联动：库类型没这能力 → 覆盖项**不返回、不生效**（历史残留覆写也不会「活过来」）；`features.SETTING_CAPS` 是唯一真值源。接口：`GET/PUT/DELETE /api/libraries/{lid}/settings`（`?keys=` 按项恢复）。
...
- 写冲突用例的两处易错：`id_conflict_with(name, lib_id)` 只算**别的**库 → 「同库放行」用例要让该 basename **只存在于目标库**；「跨库拦截」要让同名书**只存在于另一个库**（用 mixed 库收 `.cbz` 塞进去）。
...
- **HMR 看到的最新源码 ≠ 服务端产物**：验证前必须 `npm run build && npm run deploy`（第 13 期组件就曾漏 deploy，产物比源码旧）。
...
- **本地测试实例**（**已授权直接 py_compile + 重启，无需确认**）：`OUTPUT_DIR/INPUT_DIR/CONFIG_DIR/CACHE_DIR/LOG_DIR/DATA_DIR` → `/tmp/nf-test/…`，`AUTO_WATCH=false`，auth `admin`/`test1234`，`.venv/bin/python -m uvicorn novelforge.server:app --port 8791`；重启前 `pkill -f "port 8791"`，先 `mkdir -p /tmp/nf-test/{input,output,config,cache,logs,data}`。⚠️ **8993 是用户的 Docker 容器，不要去动。**
- **UI 验证（playwright）**：装在项目 `.venv`（**不进生产依赖**），用缓存内核传 `executable_path`（目录名 `chrome-headless-shell-mac-arm64/chrome-headless-shell`，**不是** `chrome-mac/`）；直连本机实例时 **`nf_token` 必须用 `context.add_init_script` 注入**（否则首屏 401 弹登录）；第 10 期迁移确认弹窗（`div.fixed.inset-0`）会挡点击 → 先 `force=True` 点「暂不迁移」。长脚本先 `write_to_file` 再 `cat` 执行，跑完删（workspace 外要 `rm`）。
...
- `docker-compose.yml`（真实版）：拉 ghcr 镜像，端口 **8992**，挂 `./data`。`docker-compose.test.yml`（测试版）：本地 build + 挂 `./novelforge:/app/novelforge`，端口 **8993**，数据隔离 `./data-test`。**断网无法 `--build`**（`docker-compose.override.yml` 是当年离线用的临时方案，联网后应删）。

## 各期交付索引（细节见同日日记）
- **6–7 期**：前端快赢（外观浮层/作者系列页/`/whats-new`+`/docs`/BookDock 拖拽）+ B 类轻后端（多格式上传、用户菜单、BookDock 五态、元数据完整度评分）；compose 简化为真实版 + 测试版。
- **8 期**：元数据在线优先（override > online > opf）+ 作者传记/头像 + ISBN 精确匹配。
- **9 期**：CBR（zip/rar 双后端 + `libarchive-tools`，缺解压器 503）+ 有声书（目录 = 一本书，`/listen/:id` 播放器，音频路由自带 Range 206）。
- **10 期**：多书库 D8 + C2（系列按媒体分组）；见上文四项决策。
- **11 期**：工程护栏 —— `tests/`（现 171 passed）+ **删除 C1**。
- **12 期**：系列级元数据（C3 + D4 遗留）；**路线图最后一项已关**。
- **13 期（已完结）**：每库覆盖 + 跨库同名防护 + 工具页库维度；已补测试与 UI 冒烟，`f07012e` 为收尾修复 commit。
