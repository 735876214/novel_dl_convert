...
- **侧栏**：品牌区（「书」字方块 +「书籍轨道」，无副标题）→ 主导航 → 四个可折叠组，无 footer。
  主导航＝仪表盘 / 探索发现 / 任务中心；**「工具」不再是独立区域**，而是**唯一一个缩进条目**
  挂在「任务中心」下面（无组标题、无分隔线，靠左侧 1px 竖引导线体现从属）。用 `NavItem.children`
  表达；子条目只有一条所以**不做折叠**（父项无折叠箭头），`nav-collapsed` 的键仍是组标题、
  存储格式未变。「书架」项已删，但 `shelf` 视图保留作共同落地页。
