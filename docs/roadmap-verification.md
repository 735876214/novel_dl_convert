...
| 说法 | 实测 | 结论 |
|---|---|---|
| 设置页 48 个 | `settingsNav.ts` 中 `p('` 计数 = 48 | ✓（**现值 38**：第 11 期删求书页、第 17 期删 9 个已决策不做的对照页） |
...
> **不再等于现值**（当前实测：设置页 **47**、后端路由 **199**）。其中「Requests 骨架」已随 C1 决策不做而删除，该行不可复现。
>
> 另：`frontend/src/router/index.ts` 与 `data/settingsNav.ts` 里曾写「46 个子路由 / 46 个叶子页」，
> 与当时的 48 也不一致 —— 现已统一修正为**删除后的真实值 47**。
