...
> **时效说明（第 31 期复核，2026-09-20）**：本文件正文是**第 0–4 期核查的当时快照**（2026-09-17），
> 之后在若干行就地追加了后续各期更正（第 11/14/15/16/20/29/31 期字样可见）。
> **本项目完成度请以 `docs/bookorbit-capability-gap.md`（逐域现状 + 复核头，第 29–31 期已逐行核验到底）
> 与 `docs/roadmap-gaps-remaining.md`（活文档）为准**；本文件保留价值在**第 0–4 期那 27 项的核查方法与实证要点**
> （记的是「当时怎么证的」，不随代码演进而过期）。第 31 期已据 capability-gap.md 复核其中与成就分类、阅读活动相关的结论。
...
| 期 | 条目 | 实证要点 |
|---|---|---|
| 0 | 审计日志 | `activity_log` 写 actor；审计页存在；~~`/api/logs` 支持 actor 过滤~~ **〔第 32 期复核更正〕前两项为真，第三项当时不成立**：`/api/logs` 只收 `limit`/`action`/`status`/`q`（`activity_log.recent()` 同），前端 `AuditLogPage.vue` 自己也如实标注「日志接口尚未支持按 actor 过滤」—— 本行把**计划**当成了**实证**，正是本文件第 11 行那句「不采信文档里的状态标记」要防的错误，只不过这次写错的是核查者自己。**该能力已于第 32 期真正落地**（`activity_log.py:252`/`:280-282`/`:286`、`server.py:3855`、测试 `tests/test_logs_actor.py`），故本条**今天全部为真** |
...
| 阅读器 | eBook 排版已实现 **15 项**（阅读模式 / 13 档主题 / 字体 / **字重样式** / 字号 / 行高 / 内容宽度 / **文本区左右内边距** / 段落间距 / 首行缩进 / 字距 / 词距 / 分栏 / 两端对齐 / 断词）—— 上一版列在「未支持」里的**字重样式**与**文本区左右内边距**本期落地（`lib/readerPrefs.ts` 的 `fontStyle` / `gutter`）。余下两项属**口径改判而非缺口**：① 新书套用设置 —— 本项目排版偏好是全局单一来源（阅读器与设置页读写同一份），不存在「每书排版」这第二层，上游那个开关要解决的问题在这里天然成立，故不造开关；② 固定版式页宽 —— 三档页宽确实没做，但**固定版式本身已能识别**（后端读 OPF 的 `rendition:layout`，见 `core/library.py:_fixed_layout_of`），这类书在阅读器里停用重排设置、页宽交给书本身决定（**〔第 36 期改判〕**见上）。**〔第 33 期改判〕**原写「有声书播放器（§11 不建议做）、CBR（需 RAR 系统依赖）」**两项均于第 9 期落地**：有声书 = `/listen/:id`（`router/index.ts:165`）+ `views/AudioPlayerView.vue`；CBR = `.cbr` 在 `core/comics.py:31` 的 `COMIC_EXTS` 里，`:48` 有 RAR 后端可用性探测（`rarfile` + 外部解压器），**缺解压器时如实报错而非静默失败** —— 「需系统依赖」是**可选前置**，不是「未支持」 |
| 维护页 | IMPORT（从其它书库工具一次性导入）/ RECOMMENDATIONS（刷新推荐索引）/ UPDATES（查 GitHub 新版本）在容器部署口径下未支持；ACHIEVEMENTS 的 Backfill 已接入维护页 |
| 外观（Icons / Layout / Behavior） | **〔第 32 期复核改判〕Layout 与 Behavior 已做实**（`views/settings/pages/LayoutPage.vue` / `BehaviorPage.vue`，`settingsNav` 由 `placeholder` 改 `ready`，每项都真的驱动书架 / 作者页）：封面尺寸 / 网格间距 / 卡片信息位置 / 作者封面尺寸与形状 / 斑马纹、缩略图点击行为 / 筛选预览默认展开 / 系列默认折叠。**仍未支持**：Icons 页（**主动不做** —— 本项目图标是内联 SVG 常量集 `lib/icons.ts`，做「风格 / 上传」需多套图标集 + 存储覆盖机制，成本远超收益，保持 `placeholder` 如实标注）；切视图独立尺寸与方形封面尺寸 / 间距（上游有、本项目无对应实体）、折叠系列封面形态与卡片主次标签可选（暂缓）—— 四项集中列在 Layout 页尾对照卡 |
...
> **〔第 32 期复核补记〕** 上句需收窄：27 项里**有 1 项（第 0 期「审计日志」）的实证要点含不实成分** ——「`/api/logs` 支持 actor 过滤」在当时（乃至到第 31 期）都**不成立**，属「把计划写成了实证」。其余 26 项本轮复核无误；该项的能力缺口已于第 32 期补齐，现已为真。这条**留在这里而不删**，是因为它正是本文件「不采信文档里的状态标记」最该防的错误 —— 只不过这次写错的是核查者自己。
