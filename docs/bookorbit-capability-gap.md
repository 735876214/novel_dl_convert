...
| 跨实体浏览（作者 / 系列 / 题材 / 出版社 / 语言 / 收藏） | 上游 `catalog.service.ts`：实体维度检索 + 收藏 | **已落地（第 34 期）**：新页 `/browse`「实体总览」（`frontend/src/router/index.ts:174`），侧栏「浏览」组首项（`frontend/src/data/nav.ts:72`）；六个维度定义在 `frontend/src/views/BrowseView.vue:51`。数据全部来自 `library.scopedBooks`（= `/api/books` 按当前书库过滤，`frontend/src/stores/library.ts:66`），**不新增聚合接口**；「收藏」维度的成员关系由 `/api/books` 逐本附带 `collection_ids`（`novelforge/server.py:992`，一次批量查询 `novelforge/core/db.py:1063` `collection_map()`） | **已落地** | ⚠️ 与 `/explore`「探索发现」是两件事 —— 那个是**外部书源检索**（`POST /api/search`），本页零外网。本项目**只有 `tags`（OPF 的 `dc:subject`）一个题材类字段**（上游分 genre / tag 两个）⇒ 只做一个「题材」维度，**不把同一份数据换个名字列两遍**；演播者维度无实体（第 34 期明确不做） |
...
| 重置阅读状态（从头开始） | 上游 `reading-state.service.ts`：`POST /books/:bookId/reset-reading-state` | **已落地（第 34 期）**：`db.reset_reading_state`（`novelforge/core/db.py:3472`）+ `POST /api/books/{bid}/reset-reading-state`（`novelforge/server.py:1727`，未知书 404、并写一条「重置」审计日志）；入口在详情页「我的记录」（`frontend/src/components/book/ReadingRecord.vue:137`、`:259`；api 方法 `frontend/src/lib/api.ts:2211`） | **已落地** | 只删**读出来的痕迹**（`reading_sessions` / `progress` / `reading_status` 三处）；批注 / 书签 / 评分 / 收藏 / 元数据覆盖与**已解锁成就**一律不动（成就的既定机制是「只解锁不回退」）；**只动 DB，不碰磁盘文件** —— 有文件指纹断言钉住 |
...
| My Reading（Started / In Progress / Completed / **Avg Progress**） | — | **已有**：前三项在 `reading` 块（`core/stats.py:666-677`）+ **`avg_progress`**（`:660`） | **已落地** | 档位更正（第 32 期） |
...
| Format Distribution | — | **已有**（`core/stats.py:635` 的 `books.by_format`）；**第 32 期另补 `by_format_size`**（按格式算体积，供 Storage by Format 图） | **已落地** | 档位更正（第 32 期） |
...
| 能力项 | 线上形态 | 本项目现状 | 档位 | 理由 |
| --- | --- | --- | --- | --- |
| Notifications 浮层 | 浮层 + 已读 + Mark all read | **已有**：头部铃铛挂浮层（`AppHeader.vue:89` → `components/NotificationBell.vue`），「全部已读」在 `NotificationBell.vue:137`；已读走后端 `POST /api/notifications/read`（`server.py:4486`）+ `notifications_read` 表（`core/db.py:258`）。整页日志视图保留，按类别偏好做客户端过滤（`lib/notifyPrefs.ts`） | **已落地** | 第 32 期再校锚点（`server.py` 后移 55 行）；第 37 期复校 ⇒ `:4486`。**Clear 仍不做**：本项目**无通知产生端**，且清空日志入口已在 Watcher / Logs 页（见 §1 该行） |
...
| 状态标签（All / Needs review / Pending / Ready / Error） | 5 态状态机 | **已有状态机**：`book_dock_items` 表（`core/db.py:338`，含 `status` 字段 + `idx_dock_status` 索引 `:350`）；五态枚举 `DOCK_TABS = ("all","needs_review","pending","ready","error")`（`core/db.py:2262`），中文标签 `core/bookdock.py:34-39`，计数装配 `bookdock.py:139-150`，前端渲染 `BookDockPage.vue:312-323` | **已落地** | 结论成立，锚点**第 35 期再核重取**：`:227`→`:248`、`:1546`→`:2114`（原记「均仍准确」已被推翻 —— 五态枚举那次补锚点补在了 `unlock_achievement` 上）；**〔第 36 期〕**再核：`:2114`→`:2160`（本期在 `db.py` 上加过东西，又后移 46 行）。⚠️ 漏报复盘：`DOCK_TABS = ("all","needs_review",…)` 里的逗号曾被核验工具当小句分隔符，把符号整段吞掉 ⇒ 已修 |
...
- **阅读状态 + 起止日期 + 书评 + 相似书 + Reading Log（按书）+ 手工补录**：`progress` 扩字段 / 新表 `reviews` / 新模块 `core/recommend.py` / 加 `GET /api/books/{bid}/reading-log` → 详情页五标签逐步补齐 —— ✅ **已完成**；⚠️ 两处与计划不同：**没有新建 `reviews` 表**（复用既有 `ratings` 表加列，`db.set_review` `core/db.py:1991`）；**没有建 `GET /api/books/{bid}/reading-log`**（实现为 `GET /api/reading-log` 返回 `items`/`by_book`/`recent` 三块，`server.py:1544-1593`）。相似书模块 `core/recommend.py` **按计划建了**
