...
| 状态标签（All / Needs review / Pending / Ready / Error） | 5 态状态机 | **已有状态机**：`book_dock_items` 表（`core/db.py:355`，含 `status` 字段 + `idx_dock_status` 索引 `:367`）；五态枚举 `DOCK_TABS = ("all","needs_review","pending","ready","error")`（`core/db.py:2363`），中文标签 `core/bookdock.py:34-39`，计数装配 `bookdock.py:139-150`，前端渲染 `BookDockPage.vue:330-342` | **已落地** | 结论成立，锚点**第 35 期再核重取**：`:227`→`:248`、`:1546`→`:2114`（原记「均仍准确」已被推翻 —— 五态枚举那次补锚点补在了 `unlock_achievement` 上）；**〔第 36 期〕**再核：`:2114`→`:2160`（本期在 `db.py` 上加过东西，又后移 46 行）。⚠️ 漏报复盘：`DOCK_TABS = ("all","needs_review",…)` 里的逗号曾被核验工具当小句分隔符，把符号整段吞掉 ⇒ 已修 |
