...
- 后端 `novelforge/server.py` 共 **270 个路由**（`grep -cE "^@app\.(get|post|put|delete|patch)\("`；第 29 期记 260 —— 第 31 期 +`GET /api/reading-activity`、第 32 期 +`POST /api/authors/{name}/sort-name`、第 34 期 +8：书签六条 + `POST /api/books/{bid}/reset-reading-state` + `GET /api/browse-counts`）；前端路由见 `frontend/src/router/index.ts`
...
| YOUR REVIEW（书评 + 评分） | 可写书评 | **已落地**：后端 `GET`/`PUT /api/books/{bid}/review`（`server.py:1508-1528`，stars 与 review 一起保存，0/空串=清除）；落 `ratings` 表 review 列（`db.set_review` `core/db.py:1843`、`db.get_review` `:1863`）；前端「我的评分与书评」卡（5 星 + 清除 + textarea + 保存）`ReadingRecord.vue:175-221`；书架列表/表格也展示评分（`ShelfView.vue:697-699,767-769`） | **已落地** | 原判「无」「需新建 `reviews` 表」均过期 —— **没有新建表**，复用了既有的 `ratings` 表加列 |
...
| 重置阅读状态（从头开始） | 上游 `reading-state.service.ts`：`POST /books/:bookId/reset-reading-state` | **已落地（第 34 期）**：`db.reset_reading_state`（`novelforge/core/db.py:3281`）+ `POST /api/books/{bid}/reset-reading-state`（`novelforge/server.py:1727`，未知书 404、并写一条「重置」审计日志）；入口在详情页「我的记录」（`frontend/src/components/book/ReadingRecord.vue:137`、`:259`；api 方法 `frontend/src/lib/api.ts:2211`） | **已落地** | 只删**读出来的痕迹**（`reading_sessions` / `progress` / `reading_status` 三处）；批注 / 书签 / 评分 / 收藏 / 元数据覆盖与**已解锁成就**一律不动（成就的既定机制是「只解锁不回退」）；**只动 DB，不碰磁盘文件** —— 有文件指纹断言钉住 |
...
| 能力项 | 线上形态 | 本项目现状 | 档位 | 理由 |
| --- | --- | --- | --- | --- |
| Notifications 浮层 | 浮层 + 已读 + Mark all read | **已有**：头部铃铛挂浮层（`AppHeader.vue:89` → `components/NotificationBell.vue`），「全部已读」在 `NotificationBell.vue:137`；已读走后端 `POST /api/notifications/read`（`server.py:4249`）+ `notifications_read` 表（`core/db.py:156`）。整页日志视图保留，按类别偏好做客户端过滤（`lib/notifyPrefs.ts`） | **已落地** | 第 32 期再校锚点（`server.py` 后移 55 行）。**Clear 仍不做**：本项目**无通知产生端**，且清空日志入口已在 Watcher / Logs 页（见 §1 该行） |
...
| 状态标签（All / Needs review / Pending / Ready / Error） | 5 态状态机 | **已有状态机**：`book_dock_items` 表（`core/db.py:236`，含 `status` 字段 + `idx_dock_status` 索引 `:248`）；五态枚举 `DOCK_TABS = ("all","needs_review","pending","ready","error")`（`core/db.py:2114`），中文标签 `core/bookdock.py:34-39`，计数装配 `bookdock.py:139-150`，前端渲染 `BookDockPage.vue:312-323` | **已落地** | 结论成立，锚点**第 35 期再核重取**：`:227`→`:248`、`:1546`→`:2114`（原记「均仍准确」已被推翻 —— 五态枚举那次补锚点补在了 `unlock_achievement` 上）。⚠️ 漏报复盘：`DOCK_TABS = ("all","needs_review",…)` 里的逗号曾被核验工具当小句分隔符，把符号整段吞掉 ⇒ 已修 |
...
- **阅读状态 + 起止日期 + 书评 + 相似书 + Reading Log（按书）+ 手工补录**：`progress` 扩字段 / 新表 `reviews` / 新模块 `core/recommend.py` / 加 `GET /api/books/{bid}/reading-log` → 详情页五标签逐步补齐 —— ✅ **已完成**；⚠️ 两处与计划不同：**没有新建 `reviews` 表**（复用既有 `ratings` 表加列，`db.set_review` `core/db.py:1843`）；**没有建 `GET /api/books/{bid}/reading-log`**（实现为 `GET /api/reading-log` 返回 `items`/`by_book`/`recent` 三块，`server.py:1544-1593`）。相似书模块 `core/recommend.py` **按计划建了**
