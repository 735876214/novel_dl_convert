...
| 跨实体浏览（作者 / 系列 / 题材 / 出版社 / 语言 / 收藏） | 上游 `catalog.service.ts`：实体维度检索 + 收藏 | **已落地（第 34 期）**：新页 `/browse`「实体总览」（`frontend/src/router/index.ts:174`），侧栏「浏览」组首项（`frontend/src/data/nav.ts:72`）；六个维度定义在 `frontend/src/views/BrowseView.vue:51`。数据全部来自 `library.scopedBooks`（= `/api/books` 按当前书库过滤，`frontend/src/stores/library.ts:66`），**不新增聚合接口**；「收藏」维度的成员关系由 `/api/books` 逐本附带 `collection_ids`（`novelforge/server.py:981`，一次批量查询 `novelforge/core/db.py:909` `collection_map()`） | **已落地** | ⚠️ 与 `/explore`「探索发现」是两件事 —— 那个是**外部书源检索**（`POST /api/search`），本页零外网。本项目**只有 `tags`（OPF 的 `dc:subject`）一个题材类字段**（上游分 genre / tag 两个）⇒ 只做一个「题材」维度，**不把同一份数据换个名字列两遍**；演播者维度无实体（第 34 期明确不做） |
| BROWSE 组计数徽标 | `browse-counts.service.ts`：侧栏三计数 + 60 s 缓存 | **已落地（第 34 期）**：`novelforge/core/browse_counts.py`（`CACHE_TTL` `:25`、`counts()` `:65`）+ `GET /api/browse-counts`（`novelforge/server.py:3359`）；前端由 `data/nav.ts` 的 `countSource: 'browse'` 声明、`components/AppSidebar.vue` 的 `navCount()` 消费 | **已落地** | 计数与目标页**同源**（作者 / 系列由 `library.books()` 聚合、批注走 `db.annotation_counts()` 只算活跃）；**刻意不带 `library_id`** —— 作者 / 系列 / 批注三页目前都是跨库的，计数跨库才对得上（传当前库会出现「侧栏 3、页面 12」）；读失败时**不显示胶囊**（显示 0 会与「真的没有」混淆）。⚠️ 未塞进 `/api/stats`：那个接口的既有键有测试钉住、且不能缓存 |
...
| 重置阅读状态（从头开始） | 上游 `reading-state.service.ts`：`POST /books/:bookId/reset-reading-state` | **已落地（第 34 期）**：`db.reset_reading_state`（`novelforge/core/db.py:2896`）+ `POST /api/books/{bid}/reset-reading-state`（`novelforge/server.py:1518`，未知书 404、并写一条「重置」审计日志）；入口在详情页「我的记录」（`frontend/src/components/book/ReadingRecord.vue:137`、`:259`；api 方法 `frontend/src/lib/api.ts:2211`） | **已落地** | 只删**读出来的痕迹**（`reading_sessions` / `progress` / `reading_status` 三处）；批注 / 书签 / 评分 / 收藏 / 元数据覆盖与**已解锁成就**一律不动（成就的既定机制是「只解锁不回退」）；**只动 DB，不碰磁盘文件** —— 有文件指纹断言钉住 |
